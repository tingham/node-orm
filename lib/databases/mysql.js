...
DBClient.prototype._insertRecord = function (collection, data, callback) {
	...
	for (k in data) {
		...
		switch (typeof data[k]) {
			...
			default:
				if( data[k] == "undefined" ){
					console.log("Field value is not set, nothing to sanitize.");
					break;
				}
				var ar_values = data[k].split("'");
				data[k] = ar_values.join("\\'");
				_values.push("'" + data[k] + "'");
...
DBClient.prototype._addQueryConditions = function (query, conditions) {
	...
	for (k in conditions) {
		...
		switch (typeof conditions[k]) {
			case "number":
				// @TODO: Replace this with a more elegant preg match set looking for many of the mysql built-ins.
				// @TODO: Have to research this but the instance of might be date in this case; entirely different process.
				/*
					Date Specific:
					--
					ADDDATE(), ADDTIME(), CONVERT_TZ(), CURDATE(), CURRENT_DATE(), CURRENT_DATE, CURRENT_TIME(), CURRENT_TIME, CURRENT_TIMESTAMP(), CURRENT_TIMESTAMP, CURTIME(), DATE_ADD(), DATE_FORMAT(), DATE_SUB(), DATE(), DATEDIFF(), DAY(), DAYNAME(), DAYOFMONTH(), DAYOFWEEK(), DAYOFYEAR(), EXTRACT(), FROM_DAYS(), FROM_UNIXTIME(), GET_FORMAT(), HOUR(), LAST_DAY, LOCALTIME(), LOCALTIME, LOCALTIMESTAMP, LOCALTIMESTAMP(), MAKEDATE(), MAKETIME, MICROSECOND(), MINUTE(), MONTH(), MONTHNAME(), NOW(), PERIOD_ADD(), PERIOD_DIFF(), QUARTER(), SEC_TO_TIME(), SECOND(), STR_TO_DATE(), SUBDATE(), SUBTIME(), SYSDATE(), TIME_FORMAT(), TIME_TO_SEC(), TIME(), TIMEDIFF(), TIMESTAMP(), TIMESTAMPADD(), TIMESTAMPDIFF(), TO_DAYS(), UNIX_TIMESTAMP(), UTC_DATE(), UTC_TIME(), UTC_TIMESTAMP(), WEEK(), WEEKDAY(), WEEKOFYEAR(), YEAR(), YEARWEEK()

					Probably not all of these apply, but any of the items that return numerics will need to be accounted for.
				*/
				if( prop.indexOf("MONTH") > -1 || prop.indexOf("DAY") > -1 || prop.indexOf("YEAR") > -1 ){
			...
			case "boolean":
				...
				break;
			default:
				// @TODO: conditions[k] may be an object type that we can't deal with due to user error, should report appropriately.
